import { render, createVNode, VNode, nextTick } from 'vue';
...
const messageInstance: IMessage = (options) => {
  // ------------- 判断参数类型 以及创建真实的dom -----------------
  if ( typeof options === 'string' ) options = { message: options, iconClass: 'success' };
  ...
  nextTick(() => {
    messageQueue.push({ vm: vNode, $el: element, id: removedId, height: vNode.el.offsetHeight});
  })
  messageId ++;
};
...
const useClose = (id: number) => {
  const localIndex = messageQueue.findIndex((item: IMessageQueue) => item.id === id);
  if ( localIndex === - 1 ) return;
  const { $el, vm, height: removedHeight } = messageQueue[localIndex];
  ...
  for ( ; i < messageQueue.length; i ++ ) {
    const { vm } = messageQueue[i],
      pos = parseInt(vm.el.style['top'], 10) - removedHeight - 16;
