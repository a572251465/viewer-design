...
const dialog:IDialog = (options) => {
  ...
  // ---------------------- 重写beforeClose方法 ------------------
  const userBeforClose = (options as IDialogOptional).beforeClose
  options = {
    ...options,
    beforeClose: () => {
      // ----------------------- 使用promise 异步删除元素 ----------------
      const p = Promise.resolve(userBeforClose())
      p.finally(() => {
...
([ 'success', 'warning', 'danger', 'info', 'confirm' ] as const).forEach(classify => {
