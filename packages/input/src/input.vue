<template>
  <div class="cu-input" :style="computedStyles">
    ...
    <span class="cu-input__suffix">
      ...
      <i
        ...
        @click="suffixIconHandle"
        v-if="(computedSuffixIcon || (clearable && changeValue)) && isNotArea"
      ></i>
    </span>
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const inputHeight = computed<number>(() =>
      props.type === 'textarea' ? 80 : 40
    )
    // 计算class
    ...
    const countComputed = computed<number>(
      () => `${changeValue.value}`.length || 0
    )
    watch(
      () => props.modelValue,
      (val) => (changeValue.value = val)
    )

    // 表示前后icon图标
    const suffixIcon = ref<string>(
      props.type === 'password'
        ? 'close-eye'
        : props.clearable
        ? 'close-circular'
    ...
    const computedPrefixIcon = computed<string>(() =>
      props.prefixIcon ? `cu-icon-${props.prefixIcon}` : ''
    )
    const computedSuffixIcon = computed<string>(() =>
      suffixIcon.value ? `cu-icon-${suffixIcon.value}` : ''
    )

    // 表示input特殊的后缀padding
    const inputSpecialStyle = computed(() => ({
      paddingRight:
        suffixIcon.value && props.showWordLimit
          ? '50px'
          : suffixIcon.value
          ? '25px'
          : '44px'
    ...
    const suffixIconHandle = () => {
      ...
      if (props.type === 'password') {
        ...
        suffixIcon.value =
          suffixIcon.value === 'close-eye' ? 'eye' : 'close-eye'
        return
      }
