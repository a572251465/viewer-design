<template>
  <div class = "icon-demo">
    ...
    <h3>图标集合</h3>
    <div class = "input">
      <cu-input :width = 500 v-model = 'inputKeyWord' placeholder = '请输入英文关键字搜索，比如：success' suffixIcon = 'search'/>
    </div>
    <ul>
...
export default defineComponent({
  ...
  setup() {
    ...
    const iconList = computed<string[]>(() => {
      ...
      return icons.filter(icon => icon.includes(inputKeyWord.value)).map(icon => `cu-icon-${ icon }`)
    })
