<template>
  <div class = "drawer-demo">
    ...
    <code-source :showFlag = true :targetCode = 'example1'>
      <cu-button class = 'margin-right' v-for = "item in ['bottom', 'top', 'right', 'left']" :key = 'item'
                 @click = "openDrawer(item)">{{ item }}
      </cu-button>
    ...
    <code-source :showFlag = false :targetCode = 'example2'>
      ...
      <cu-drawer v-model = 'showFlag2' :clickMaskClose = true :closeOnPressEscape = true>
        <div class = "flex">
          <span>account：</span>
          <cu-input placeholder = '请输入账号'/>
        </div>
...
import { example, example1, example2, example3, componentProps, eventProps } from './drawer-data'
