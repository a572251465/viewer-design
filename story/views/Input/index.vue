<template>
  <div class = "demo-input">
    ...
    <code-source :showFlag = false :targetCode = 'example1'>
      <div v-for = "item in inputTypeList" :key = "item.type" style = "margin-top: 15px">
        <cu-input v-model = 'item.value' :type = 'item.type' :placeholder = 'item.placeholder'/>
      </div>
    </code-source>
    <h3>不同大小</h3>
    <code-source :showFlag = false :targetCode = 'example2'>
      <div v-for = "item in ['big', 'medium', 'small', 'mini']" :key = "item" style = "margin-top: 15px">
    ...
    <code-source :showFlag = false :targetCode = 'example5'>
      <div>
        <cu-input placeholder = '请输入账号' prefixIcon = 'edit' suffixIcon = 'search'/>
      </div>
    </code-source>
    <h3>自动获取光标</h3>
...
import { example, example1, example2, example3, example4, example5, example6, componentProps, eventProps } from './input-data'
